...
def plot_nominal_learning_curve(csv_path: str = "results/nominal_learning_curve.csv", output_dir: str = "results"):
    ...
    gs = GridSpec(2, 2, height_ratios=[1.2, 1.0], hspace=0.28, wspace=0.25)
...
def plot_sensitivity_analysis(csv_path: str = "results/sensitivity_analysis.csv", output_dir: str = "results"):
    ...
    for cat in categories:
        ...
        fig_single, ax_single = plt.subplots(figsize=(7, 4.8))
        param_values = cat_df["value"].unique()
        ...
        for idx, val in enumerate(param_values):
            ...
            ax_single.plot(sub["timestep"], sub["mean_reward"], color=color, label=label_name, lw=2.4)
        ...
        ax_single.axhline(y=200, color="#AE2012", linestyle=":", lw=1.8, label=r"Solved Benchmark ($R \geq 200$)")
        ax_single.axhline(y=0, color="gray", linestyle="-", alpha=0.5, lw=0.8)
        
        # ax_single.set_title(f"Sensitivity Study: {cat}")
        ax_single.set_xlabel(r"Environment Timesteps ($t$)")
        ax_single.set_ylabel(r"Mean Evaluation Return $\mathbb{E}[R]$")
        ax_single.grid(True)
        ax_single.legend(loc="lower right", framealpha=0.92)
...
def plot_trajectory_profile(model_path: str = "models/best_model.zip", output_dir: str = "results"):
    ...
    fig, axes = plt.subplots(3, 2, figsize=(10.5, 8.0), sharex=True)
...
def plot_convergence_bar_chart(summary_csv: str = "results/convergence_summary_table.csv", output_dir: str = "results"):
    ...
    Excludes non-converged runs, borderless bars, optimized legend placement.
    """
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, 4.5), gridspec_kw={"wspace": 0.35})
    ...
    # plt.suptitle(r"\textbf{Hyperparameter Convergence Comparison: Time-to-Threshold ($R \geq 190$)}", fontsize=13, y=1.03)
